...
//  Drawing Fun
//
//  Created by Joy Biswas on 3/12/17.
//  Copyright © 2017 Joy Biswas. All rights reserved.
//

import UIKit

class ViewController: UIViewController,ColorSentDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
       tool = UIImageView()
        tool.frame = CGRect(x: self.view.bounds.height, y: self.view.bounds.width, width: 18, height: 18)
        tool.image = #imageLiteral(resourceName: "paint_brush1x")
    ...
    @IBAction func eraser(_ sender: AnyObject) {
        if(isDrawing) {
            ...
            tool.image = #imageLiteral(resourceName: "Ereaser")
            toolIcon.setImage(#imageLiteral(resourceName: "paint_brush1x"), for: .normal)
        }
        else {
            (red,green,blue) = (0,0,0)
            tool.image = #imageLiteral(resourceName: "paint_brush1x")
            toolIcon.setImage(#imageLiteral(resourceName: "Ereaser"), for: .normal)
...
extension ViewController:UINavigationControllerDelegate,UIImagePickerControllerDelegate,SettingVCDelegate {
